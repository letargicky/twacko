...
app.post("/interests/add", (req, res) => {
  const { userId, interests: newInterests } = req.body;
  const user = users.find((u) => u.id === userId);
  if (!user) {
    return res.status(404).json({ error: "User not found." });
  }

  const userInterests = interests.find((i) => i.userId === userId);
  if (userInterests) {
    userInterests.interests.push(...newInterests);
  } else {
    interests.push({ userId, interests: newInterests });
  }

  res.status(201).send({ userId, interests: newInterests });
});

// Delete interests for a user
app.post("/interests/delete", (req, res) => {
  const { userId } = req.body;
  const interestIndex = interests.findIndex((i) => i.userId === userId);
  if (interestIndex === -1) {
